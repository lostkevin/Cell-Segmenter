...
def get_masks(p, bd, dist, mask, inds, cluster=False, prefix=''):
    ...
    if False:
        ...
        clusterer = DBSCAN(eps=2**0.5, min_samples=30, n_jobs=-1)
        ...
        skelmask = torch.zeros_like(dist, dtype=int)
        #! new code
        # h, w = skelmask.shape
        # data, counts = torch.unique(new_px[0] * w + new_px[1], return_counts=True)
        # y = torch.div(data, w, rounding_mode='floor')
        # x = data - y * w
        # skelmask[y, x] = counts

        skelmask[new_px] = 1
        # cv2.imwrite('/nfs4-p1/hkw/debug/skel.png', ((skelmask / skelmask.max()).cpu().numpy() * 255).astype(np.uint8))
        ...
        # cv2.imwrite(f'/nfs4-p1/hkw/debug/{prefix}skel.png', (((skelmask > 0).cpu().numpy() * 255).astype(np.uint8)))

        # connect = measure.label((skelmask > 0).cpu().numpy(), connectivity=skelmask.ndim)
        # slices = find_objects(connect)
        # drop = torch.zeros_like(skelmask)
        # for i,slc in enumerate(slices):
        #     if slc is not None:
        #         msk = skelmask[slc] == (i+1)
        #         npix = skelmask[slc][msk].sum()
        #         if npix < 5:
        #             # remove fragment
        #             drop[slc][msk] = 1
        # skelmask[drop] = 0
        # labels = measure.label((skelmask > 0).cpu().numpy(), connectivity=skelmask.ndim)
...
def compute_masks(
    ...
    if debug:
        cv2.imwrite(os.path.join(debug_dir, f'{filename_noext}binary_mask.png'), mask.cpu().numpy() * 255)

        # vis_dist = interpolate(dist, torch.tensor([dist.min(), dist.max()], device=dist.device), torch.tensor([0, 1], device=dist.device))
        # cv2.imwrite(os.path.join(debug_dir, f'{filename_noext}edt.png'), np.clip(vis_dist.cpu().numpy() * 255, 0, 255).astype(np.uint8))

        cv2.imwrite(os.path.join(debug_dir, f'{filename_noext}edt.png'),
                    (dist.sigmoid().cpu().numpy() * 255).astype(np.uint8))

        cv2.imwrite(os.path.join(debug_dir, f'{filename_noext}boundary_mask.png'),
                    (bd.sigmoid().cpu().numpy() * 255).astype(np.uint8))
        cv2.imwrite(os.path.join(debug_dir, f'{filename_noext}boundary_mask_dir.png'),
                    ((bd > -1).cpu().numpy() * 255).astype(np.uint8))

        vis_flowY = interpolate(flows[0], torch.tensor([flows[0].min(), flows[0].max()],
                                device=dist.device), torch.tensor([0, 1], device=dist.device))
        cv2.imwrite(os.path.join(debug_dir, f'{filename_noext}flowY.png'),
                    np.clip(vis_flowY.cpu().numpy() * 255, 0, 255).astype(np.uint8))

        vis_flowX = interpolate(flows[1], torch.tensor([flows[1].min(), flows[1].max()],
                                device=dist.device), torch.tensor([0, 1], device=dist.device))
        cv2.imwrite(os.path.join(debug_dir, f'{filename_noext}flowX.png'),
                    np.clip(vis_flowX.cpu().numpy() * 255, 0, 255).astype(np.uint8))
    ...
    if mask.any():
        ...
        p = np.zeros([2, 1, 1])
        tr = []
        mask = np.zeros_like(dist.cpu().numpy()).astype(np.uint32)
    return mask, p, tr
