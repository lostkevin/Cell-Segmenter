...
def soft_nms_pytorch(dets, box_scores, sigma=0.5, thresh=0.5):
    N = dets.shape[0]
    indexes = torch.arange(0, N, dtype=torch.int, device=dets.device).view(N, 1)
    ...
    for i in range(N):
        ...
        inter = torch.tensor(w * h, device=dets.device)
...
def save_as_csv(output_path, results: Dict[str, Dict[str, float]]):
